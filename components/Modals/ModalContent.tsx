...
export default function ModalContent({ isOpen, back, isForm, closeModal, children, id, title }: Props) {
    return (
                    ...
                    <div className="flex relative justify-center p-4 text-center">
                        ...
                        >
                            <Dialog.Panel className="w-full max-w-md inset-y-0 transform overflow-hidden rounded-2xl bg-white dark:bg-secondaryDark max-h-screen overflow-y-auto p-6 text-left align-middle shadow-xl transition-all">
                                ...
                                {isForm ? <button
                                    onClick={closeModal}
                                    className='absolute right-0 border-4 dark:border-[#18181a] border-white top-0 py-2 px-4 rounded-bl-2xl rounded-tr-2xl hover:bg-indigo-200 dark:hover:bg-indigo-900/30 transition duration-200 text-lg'
                                >
